...
export function useDistributionLinks({ passId }: UseDistributionLinksOptions) {
  ...
  const createLink = async () => {
    ...
    try {
      ...
      const axiosError = err as AxiosError;
      setError(axiosError.message || 'Failed to create distribution link');
      throw err;
    } finally {
      setLoading(false);
    }
  ...
  const updateLink = async (linkId: number, status: 'active' | 'disabled') => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const axiosError = err as AxiosError;
      setError(axiosError.message || 'Failed to update distribution link');
      throw err;
    } finally {
      setLoading(false);
    }
